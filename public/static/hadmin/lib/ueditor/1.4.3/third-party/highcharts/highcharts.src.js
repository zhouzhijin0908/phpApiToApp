	...
	init: function (elem, fromD, toD) {
		...
		if (start.length) {
			...
			while (start.length < endLength) {
				...
				if (bezier) { // disable first controller point
...
Point.prototype = {
	 ...
	 *     this happens if the controller key (Cmd on Mac) was pressed during clicking.
	 */
...
var SplineSeries = extendClass(Series, {
	...
	getPointSpline: function (segment, point, i) {
		var smoothing = 1.5, // 1 means controller points midway between points, 2 means 1/3 from the point, 3 is 1/4 etc
			denom = smoothing + 1,
			plotX = point.plotX,
			plotY = point.plotY,
			lastPoint = segment[i - 1],
		...
		if (lastPoint && nextPoint) {
			...
			// have the two controller points make a straight line through main point
			...
			// to prevent false extremes, check that controller points are between
			// neighbouring points' y values
			if (leftContY > lastY && leftContY > plotY) {
				leftContY = mathMax(lastY, plotY);
				rightContY = 2 * plotY - leftContY; // mirror of left controller point
		...
		// Visualize controller points for debugging
		/*
		if (leftContX) {
